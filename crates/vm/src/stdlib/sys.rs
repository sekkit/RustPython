...
pub mod sys {
    ...
    fn dllhandle(_vm: &VirtualMachine) -> usize {
        // The Python DLL handle: ctypes.pythonapi resolves Python C-API
        // symbols from this handle. python311.dll is the forwarding shim
        // generated by bench/make_python_dll_shims.ps1; its exports forward
        // to the executable's own C-API symbols. Falls back to 0 (the ctypes
        // stub) when the shim is not available next to the interpreter.
        static HANDLE: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
        *HANDLE.get_or_init(|| {
            rustpython_host_env::ctypes::open_library("python311.dll").unwrap_or(0)
        })
    }
