...
macro_rules! extern_libpython {
    ...
    (@impl $abi:literal { $($body:tt)* } $($dll:literal),* $(,)?) => {
        ...
        // RustPython: the "pythonXY" fallback would make MSVC resolve every
        // `Py*` symbol through an import library (as `__imp_*`), conflicting
        // with rustpython-capi's own `#[unsafe(no_mangle)]` exports in the same
        // binary. Without the attribute the references are plain direct calls
        // that link against those exports.
        extern $abi {
