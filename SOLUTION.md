...
> 状态:性能/兼容性/并发三大维度全部量化;**13 项修复已验证并推送到 fork**(见 §9);生态可用性大幅提升(requests/flask/django/pytest 全通);C 扩展(.pyd)PEP 489 加载已打通,**pip 二进制 wheel 可安装可 import**,**`python -m venv` 可用且 test_venv 40 全绿**,**信号系统修复**(SIGINT 同步投递、interrupt_main 不再挂死,test_signal 57 / test_threading 229 全绿),**winreg WTF-8 代理项处理**(test_mimetypes 38 / test_winreg 25 全绿),**cProfile + profile 全修复**(sys.monitoring 可 import、_lsprof 按 monitoring 重写、c_call 事件按 CPython 过滤,test_cprofile 24 / test_profile 7 / test_sys_setprofile 29 / test_monitoring 88 全绿且**零 expectedFailure**);test_importlib 1440 全绿;JIT 实测无收益
...
| `3a6a768e2` | **fix(profile): c_call 事件只投递给内置函数/方法;专用调用指令在 tracing 下走通用路径** | profile.Profile 输出与 CPython 期望有三处差异:多余 `:0(C)`/`:0(range)` 条目(类型调用被记为 C 调用)、list.append 计数 2/4、系统性计时偏差;根因均在 VM:① `PyCallable::invoke/invoke_vectorcall` 对**所有**非 Python 函数可调用对象都发 c_call/c_return/c_exception,而 CPython `trace_call_function`(ceval.c)只对 PyCFunction/PyCMethod 与方法描述符(以首参绑定)发事件,类型/partial 等不发;新增 `trace_c_callable()`:内置函数/方法原样通过、方法描述符以首参绑定(无参则跳过)、其余不发任何 C 事件;② 自适应调用专用指令 `CallMethodDescriptorNoargs/O/Fast/FastWithKeywords`、`CallLen`、`CallIsinstance`、`CallListAppend` 直接调原生函数,绕过 trace 机制,指令被专用化后的调用对 profiler 不可见(append 2/4 的来源);tracing 激活时这些指令回退到通用 vectorcall 路径(与 CPython 在 tracing 下不专用化一致;`CallBuiltinO/Fast/FastWithKeywords`、`CallNonPyGeneral`、`CallKwNonPy` 本就走 vectorcall 无需改;类型调用专用指令 `CallType1/Str1/Tuple1/CallBuiltinClass/CallAllocAndEnterInit` 不发 c_call 与 CPython 一致);实测:profile.Profile 输出与 CPython 3.14 期望**逐字节一致**,`ProfileTest.test_cprofile` 的 expectedFailure 标记**移除**,test_cprofile.py 的包装回退为普通继承;test_profile **7/7**、test_cprofile 24/24 全绿且**零 expectedFailure**;回归:test_sys_settrace 448 / test_sys_setprofile 29 / test_monitoring 88 / test_builtin 138 / test_compile 185 / test_code 37 / test_importlib 1,440 / test_ctypes 328 / test_sysconfig 37 / test_venv 40 / test_signal 57 / test_threading 229 / test_mimetypes 38 / test_winreg 25 / test_pdb 210 / test_traceback 370 / test_inspect 362 全绿,clippy 无新告警,capi 112 pass |
