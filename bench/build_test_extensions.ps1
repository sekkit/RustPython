...
# real .pyd files linked against the RustPython C-API shim (python311.dll).
#
# Sources come from the CPython 3.14.x release tarball:
#   Modules/_testsinglephase.c  -> Lib\_testsinglephase.pyd
#   Modules/_testmultiphase.c   -> Lib\_testmultiphase.pyd
# (the multi-phase file exports all the *_bad_slot_*, *_nonmodule, *_exec_*,
#  *_export_* and non-ASCII init functions from the one DLL)
#
# The modules must be findable as top-level modules on sys.path (the
# test_importlib suite scans sys.path for '_testsinglephase.pyd'), so the
# built files land in Lib\.
#
...
# `bench\make_python_dll_shims.ps1` run so python311.lib exists in target\release.
...
foreach ($mod in @("_testsinglephase", "_testmultiphase")) {
    ...
    $out = Join-Path $root "Lib\$mod.pyd"
    ...
    if (-not (Test-Path $out)) {
        Write-Error "failed to build $mod.pyd (see compiler output above)"
    }
